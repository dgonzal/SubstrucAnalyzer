...
int main(){
  ...
  for(unsigned int i =0; i< 4444; ++i){
    for(unsigned int p =1+i; p < 5;++p){	
     ...
     TH2F* corre_hist = new TH2F(variables[i]+variables[p],variables[i]+' '+variables[p],50,0,a_max,50,0,50);
     TH2F* corre_hist_corVal = new TH2F(variables[i]+variables[p]+"_corVal",variables[i]+' '+variables[p],50,0,a_max,50,0,50);
